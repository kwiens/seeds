...
vi.mock("@/components/admin/user-list", () => ({
  UserList: ({ users }: { users: Array<{ name: string }> }) => (
...
describe("AdminPage", () => {
  beforeEach(() => {
    ...
    vi.mocked(getAllUsers).mockResolvedValue(directoryUsers);
  ...
  it("loads the People tab and passes every account to the directory", async () => {
    ...
    expect(screen.getByTestId("user-list")).toHaveTextContent("Alice Account");
  });
});
